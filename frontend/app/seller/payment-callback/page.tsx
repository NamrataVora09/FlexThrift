...
function SellerPaymentCallbackInner() {
  ...
  const checkPayment = async () => {
    ...
    if (data.status === 'success') {
      ...
      setTimeout(() => router.push('/seller/subscriptions?success=' + encodeURIComponent('Your subscription is now active!')), 2000);
      ...
      <div className="card-wrap">
        ...
        {status === 'failed' && (
          <div className="d-flex flex-column gap-2 mt-3">
            <button className="btn btn-warning fw-bold" style={{ color: '#fff' }} onClick={() => router.push('/seller/subscriptions')}>
              Back to Plans
            </button>
          </div>
        )}
...
export default function SellerPaymentCallbackPage() {
  return (
