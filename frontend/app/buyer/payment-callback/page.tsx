...
function PaymentCallbackInner() {
  ...
  const checkPayment = async () => {
    ...
    const res = await api.get<{ status: string; message: string }>(`/buyer/verify-payment?id=${merchantOrderId}`);
    // verifyPayment returns { status, message } at root — cast to access directly
    const data = res as unknown as { status: string; message: string }; // status field is at root

    if (data.status === 'success') {
      clearTimers();
      setProgress(100);
      setStatus('success');
      setMessage(data.message || 'Payment successful! Subscription activated.');
      setTimeout(() => router.push('/buyer/subscriptions?success=' + encodeURIComponent('Your subscription is now active!')), 2000);
    ...
    <>
      ...
      <div className="card-wrap">
        <div className={`coin coin-${status === 'verifying' || status === 'pending' ? 'verifying' : status}`}>
          {(status === 'verifying' || status === 'pending') && <i className="bi bi-currency-rupee" />}
        ...
        <h2 className="fw-bold mb-2" style={{ fontSize: '1.4rem' }}>
          {status === 'verifying' || status === 'pending' ? 'Confirming Payment' : status === 'success' ? 'Payment Successful!' : 'Payment Failed'}
        </h2>
        ...
        {status === 'success' && (
          <div className="alert alert-success py-2 small fw-bold">
            <i className="bi bi-check-circle me-1" /> Redirecting to your subscriptions…
          </div>
        )}

        {status === 'failed' && (
          <div className="d-flex flex-column gap-2 mt-3">
            <button className="btn btn-primary" onClick={() => router.push('/buyer/subscriptions')}>
              Back to Plans
            </button>
          </div>
        )}
