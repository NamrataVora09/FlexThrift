...
export default function CheckoutPlanPage() {
  ...
  useEffect(() => {
    if (!planId) return;
    api.get<CheckoutData>(`/buyer/plan-checkout-details/${planId}`).then((r) => {
      if (r.success && r.data) {
  ...
  const applyCoupon = useCallback(async () => {
    ...
    setCouponMsg(null);
    const res = await api.post<{ discount: number }>('/buyer/apply-coupon', {
  ...
  const processPayment = useCallback(async () => {
    ...
    // Build callback URL with {id} placeholder that PHP will replace
    const callbackUrl = `${window.location.origin}/buyer/payment-callback?id={id}`;

    const res = await api.post<{ redirect_url: string; merchant_order_id: string }>('/buyer/initiate-payment', {
      plan_id: Number(planId),
  ...
  if (loading) {
    return (
      <div
        style={{ minHeight: '100vh', display: 'flex', alignItems: 'center', justifyContent: 'center', background: '#f0f2f5' }}
      >
  ...
  return (
    <>
      ...
      <style>{`
        :root { --brand: #00a29a; --brand-dark: #008982; }
        ...
        .btn-brand { background: var(--brand); color: #fff; border: none; border-radius: 8px; font-weight: 600; padding: .75rem 1.5rem; transition: all .3s; }
        .btn-brand:hover:not(:disabled) { background: var(--brand-dark); color: #fff; transform: translateY(-2px); }
        ...
        .price-total { border-top: 1px dashed #dee2e6; margin-top: 1rem; padding-top: 1rem; font-size: 1.2rem; font-weight: 700; color: var(--brand); display: flex; justify-content: space-between; }
        .plan-badge { background: rgba(0,162,154,.1); color: var(--brand); padding: .2rem .7rem; border-radius: 2rem; font-size: .82rem; font-weight: 600; }
            ...
            <div className="card p-4 h-100">
              <div className="mb-4">
                <button
                  ...
                  onClick={() => router.push('/buyer/subscriptions')}
                >
              ...
              {/* Plan card */}
              <div className="d-flex align-items-center p-3 rounded-3 bg-light mb-4 border border-info-subtle">
                <div
                  className="bg-primary text-white p-3 rounded-circle me-3 d-flex align-items-center justify-content-center"
                ...
                <ul className="list-unstyled mb-0">
                  <li className="mb-2 small d-flex align-items-center">
                    ...
                    {plan.plan_type === 'quantity' ? (
                      <>Access to view <strong className="mx-1">{plan.limit_value}</strong> seller contact details</>
                    ) : (
                      <><strong>Unlimited</strong>&nbsp;contact views for the duration</>
                    )}
                  </li>
                  <li className="mb-2 small d-flex align-items-center">
                    <i className="bi bi-check-circle-fill text-success me-2" />
              ...
              <div className="mt-auto pt-4 border-top">
                ...
                <div className="p-3 rounded bg-light border">
                  ...
                  {user.address && (
                    <p className="mb-0 small text-muted mt-2 pt-1 border-top">{user.address}</p>
                  )}
                </div>
              </div>
            </div>
              ...
              {charge_breakdown.map((c, i) => (
                <div key={i} className="price-row">
                  <span className="text-muted">
                    {c.name}&nbsp;({c.type === 'percentage' ? `${c.value}%` : 'Fixed'})
                  </span>
