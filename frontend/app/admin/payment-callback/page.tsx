...
function AdminPaymentCallbackInner() {
  ...
  const checkPayment = async () => {
    ...
    const res = await api.get<{ status: string; message: string }>(`/admin/verify-payment?id=${merchantOrderId}`);
    ...
    if (data.status === 'success') {
      ...
      setTimeout(() => router.push('/admin/subscriptions?success=' + encodeURIComponent('Your subscription is now active!')), 2000);
